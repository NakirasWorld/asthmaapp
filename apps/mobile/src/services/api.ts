...
class SimpleApiClient {
  ...
  async login(credentials: LoginForm): Promise<ApiResponse<AuthResponse>> {
    ...
    if (response.success && response.data) {
      ...
      try {
        ...
        await storage.setItem('refresh_token', response.data.tokens.refreshToken);
      } catch (error) {
  ...
  async submitChildInfo(data: {
    ...
    return this.request<{ nextStep: string }>('/profile/onboarding/child-info', {
      method: 'POST',
  ...
  async submitMedications(data: {
    ...
    return this.request<{ nextStep: string }>('/profile/onboarding/medications', {
      method: 'POST',
      body: JSON.stringify(data),
    });
  }
  ...
  async submitNotifications(data: {
    ...
    return this.request<{ onboardingCompleted: boolean }>('/profile/onboarding/notifications', {
      method: 'POST',
      body: JSON.stringify(data),
    });
  }
  ...
  async getOnboardingStatus(): Promise<ApiResponse<{
    ...
    currentStep?: string;
    steps: Array<{ stepName: string; completed: boolean; completedAt?: string }>;
  }>> {
    return this.request<{
      onboardingCompleted: boolean;
      currentStep?: string;
  ...
  async completeOnboarding(): Promise<ApiResponse<{ onboardingCompleted: boolean }>> {
    return this.request<{ onboardingCompleted: boolean }>('/profile/onboarding/complete', {
      method: 'POST',
    });
  }
}
